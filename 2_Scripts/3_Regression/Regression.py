...
n_bad_reviews = (df['PreRelease Tomatometer Reviews'] <= 0).sum()
...
if n_bad_total > 0:
    before = len(df)
    df = df[(df['PreRelease Tomatometer Reviews'] > 0) &
            (df['Opening'] > 0) &
            (df['Budget'] > 0)].copy()
...
# Bivariate relationship between critical reception (PreRelease Tomatometer) and opening weekend revenue.
X1 = sm.add_constant(reg_df[['PreRelease Tomatometer']])
y = reg_df['ln_Opening_Weekend_Gross']
model1 = sm.OLS(y, X1).fit()
log_summary("MODEL 1: PreRelease Tomatometer only", model1)


# Step 4: Regression Model 2
# Add review volume as a control
X2 = sm.add_constant(reg_df[['PreRelease Tomatometer', 'ln_Number_of_Reviews']])
...
X3 = sm.add_constant(reg_df[['PreRelease Tomatometer', 'ln_Number_of_Reviews',
...
X_vif = sm.add_constant(reg_df[['PreRelease Tomatometer', 'ln_Number_of_Reviews',
                                 'Franchise', 'ln_Budget', 't']])
vif = pd.DataFrame()
...
X4 = sm.add_constant(df_franchise[['PreRelease Tomatometer', 'ln_Number_of_Reviews',
                                    'ln_Budget', 't']])
...
X5 = sm.add_constant(df_nonfranchise[['PreRelease Tomatometer', 'ln_Number_of_Reviews',
                                       'ln_Budget', 't']])
...
# Tests whether the effect of PreRelease Tomatometer differs between franchise and non-franchise films.
df['PreRelease_Tomatometer_Franchise'] = df['PreRelease Tomatometer'] * df['Franchise']
y_full = df['ln_Opening_Weekend_Gross']
X_int = sm.add_constant(df[['PreRelease Tomatometer', 'Franchise',
                             'PreRelease_Tomatometer_Franchise',
